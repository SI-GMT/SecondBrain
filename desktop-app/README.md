...
the user can apply a desktop release in place via the "Download &
install" button (engine in-place pip-upgrade is scaffolded but
needs the engine release to attach wheel assets — see roadmap).

Since v0.10.3 the runtime version is **baked at build time** into
`src/sb_desktop/_version.py` (generated by `build/build_windows.ps1`
from `pyproject.toml`) so a stale `.dist-info` left behind by an
earlier install on the same machine cannot make a freshly upgraded
tray report the wrong version.
