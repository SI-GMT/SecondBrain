...
def _action_status(icon: pystray.Icon, state: TrayState):
    def handler(*_args) -> None:
        _do_status_probe(icon, state, run_live=True)
...
def _action_update(icon: pystray.Icon, state: TrayState):
    def handler(*_args) -> None:
        ...
        _do_status_probe(icon, state, run_live=False)
...
def _do_status_probe(icon: pystray.Icon, state: TrayState, *, run_live: bool) -> None:
    snapshot = probe_status(run_live=run_live)
    with state.lock:
...
def run_tray() -> int:
    ...
    def _on_ready(_icon: pystray.Icon) -> None:
        _icon.visible = True
        threading.Thread(
            target=lambda: _do_status_probe(icon, state, run_live=True),
            daemon=True,
