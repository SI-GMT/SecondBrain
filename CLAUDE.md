...
**Divergences intentionnelles — catégories MCP-only (depuis v0.9.0)** : la lib MCP expose deux catégories supplémentaires qui auditent **le kit repo**, pas le vault :

- `mcp-tool-spec-drift` _(v0.9.0)_ — compare chaque `core/procedures/mem-X.md` à son hash dans `mcp-server/src/memory_kit_mcp/sync.json`.
- `skill-description-too-long` _(v0.9.x)_ — scanne `kit_repo/adapters/{claude-code,codex,mistral-vibe,copilot-cli}/skills/**/SKILL.md*` pour repérer les `description` frontmatter qui dépasseraient les 1024 chars de la limite Anthropic.

Le standalone n'a pas d'équivalent par design — il scanne des vaults sur des machines qui peuvent ne pas avoir le kit installé. Cette divergence est documentée dans l'en-tête des deux modules.
