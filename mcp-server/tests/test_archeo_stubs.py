...
async def test_total_tool_count_is_33(client: Client) -> None:
    """Sanity check: the v0.10.x milestone is 33 mem_* tools registered.
    ...
    for explicit GitHub release version interrogation).
    """
    tools = await client.list_tools()
    mem_tools = [t for t in tools if t.name == "mem" or t.name.startswith("mem_")]
    assert len(mem_tools) == 33, f"expected 33 mem_* tools, got {len(mem_tools)}: {[t.name for t in mem_tools]}"
