...
$skillsV05 = [ordered]@{
    ...
    'mem-archeo' = @{
        Description = "Triphasic archeo orchestrator. Runs Phase 1 (organizational/decisional/functional context — mem-archeo-context), Phase 2 (technical stack — mem-archeo-stack), Phase 3 (Git history — mem-archeo-git) in sequence on a Git repo, sharing a single Phase 0 topology scan across them. AUTO-TRIGGER (without waiting for /mem-archeo) when the user says — 'do a full archeo of this project', 'reconstruct project context, stack and history', 'archeo on this repo'. Also invocable via /mem-archeo [repo-path] with options --skip-phase, --only-phase, --level, --since, --until, --window, --rescan, --dry-run, --no-confirm. Persists a topology snapshot in 99-meta/repo-topology/{slug}.md."
        ArgsText = "Optional: path to the Git repo (default: CWD). Options: --project {slug}, --skip-phase context|stack|git, --only-phase context|stack|git, --level tags|releases|merges|commits, --since YYYY-MM-DD, --until YYYY-MM-DD, --window day|week|month, --depth N, --rescan, --dry-run, --no-confirm."
    }
    'mem-archeo-context' = @{
        ...
        ArgsText = "Optional: path to the Git repo (default: CWD). Options: --project {slug}, --depth N, --only-categories workflow,sync,multi-tenant,security,adr,goal,other, --rescan, --dry-run, --no-confirm."
    }
    'mem-archeo-stack' = @{
        ...
        ArgsText = "Optional: path to the Git repo (default: CWD). Options: --project {slug}, --depth N, --only-layers frontend,backend,db,ci,infra,tests,tooling,other, --rescan, --dry-run, --no-confirm."
    }
    'mem-archeo-git' = @{
        Description = "Phase 3 of the triphasic archeo: reconstruct the Git history of an existing repo as dated archives, enriched by the topology and stack from Phases 0/1/2. Surfaces friction sequences (>=3 successive commits on the same theme) and cross-links derived atoms. Source archeo-git. AUTO-TRIGGER (without waiting for /mem-archeo-git) when the user says — 'do a Git retro of this project', 'reconstruct the history', 'archeo the commits'. Also invocable directly. Idempotent via (project, source_milestone, source_atom_type, source_atom_subject)."
        ArgsText = "Optional: path to the Git repo (default: CWD). Options: --level tags|releases|merges|commits, --project {slug}, --since YYYY-MM-DD, --until YYYY-MM-DD, --window day|week|month, --depth N, --rescan, --dry-run, --no-confirm."
    }
