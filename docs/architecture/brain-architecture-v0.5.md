...
- **Préparation à CollectiveBrain** : le scope `pro` + flag `collectif` devient la clé de voûte de la future synchronisation entreprise.
...
**Décision 2026-04-24** : principes et objectifs sont **deux zones distinctes** (40-principes, 50-objectifs), parce que *comment je choisis* ≠ *vers quoi je vais*. Les fusionner (dans une zone « volitive ») masquerait cette différence fonctionnelle.
...
| `40-principes` | Fonctions exécutives volitives — normes d'action |
| `50-objectifs` | Fonctions exécutives prospectives — intentions |
| `60-personnes` | Cognition sociale — carte relationnelle |
...
```yaml
scope: perso          # ou pro — obligatoire, valeur par défaut = pro
collectif: false      # ou true — optionnel, n'a de sens que si scope: pro
                      # défaut = false — non lu par SecondBrain en v0.5
```

**Règle d'invariant** : si `scope: perso`, alors `collectif: false` (toujours). Le plugin CollectiveBrain ignorera tout fichier `perso` quel que soit le flag.
...
| `10-episodes/projets/*` | oui | oui | un projet est soit perso soit pro, jamais les deux |
| `10-episodes/vie` | oui | rare | événements de vie, typiquement perso |
| `20-knowledge/metier` | rare | oui | connaissance professionnelle |
| `20-knowledge/tech` | rare | oui | connaissance technique (typiquement pro, sauf loisir tech) |
| `20-knowledge/vie` | oui | rare | connaissance perso (santé, parentalité, langues...) |
| `30-procedures` | oui | oui | chaque procédure a son scope |
| `40-principes` | oui | oui | principes perso (lignes rouges de vie) vs principes pro (règles métier) |
| `50-objectifs` | oui | oui | objectifs de vie perso vs roadmaps pro |
| `60-personnes` | oui | oui | famille / amis (perso) vs collègues / clients (pro) |
...
**Use case A — utilisateur perso pur.** Une personne qui utilise SecondBrain pour gérer sa vie personnelle uniquement (projets perso, journal, lectures, objectifs de vie). Quasi-totalité du vault en `scope: perso`. Le flag `collectif` n'est jamais activé. CollectiveBrain n'est pas installé. C'est le cas le plus simple.

**Use case B — utilisateur pro en entreprise.** Une personne qui utilise SecondBrain comme mémoire professionnelle dans le cadre de son travail (projets d'équipe, procédures internes, compétences métier, contacts pros, connaissances techniques). Majorité du vault en `scope: pro`. Le scope `perso` est **réduit à la facette professionnelle de la personne** : préférences de travail, style de collaboration, éléments de contexte individuel utiles dans le contexte pro (« je travaille mieux le matin », « j'ai une expertise Y rarement mobilisée », « mon engagement sur le projet X se termine en juin »). Flag `collectif: true` activé sur les contenus à remonter vers le vault collectif de l'entreprise.

**Use case C — hybride.** Une personne qui utilise un **seul vault SecondBrain** pour sa vie perso et pro. Les deux scopes coexistent, cloisonnés par frontmatter. Les skills filtreront selon le scope au moment de la recherche / rappel / archivage (paramètre `--scope perso|pro|all`). CollectiveBrain ne voit que les contenus `pro` flaggués `collectif: true`.
...
- `/mem-archive` (au moment d'écrire) — doit demander ou inférer le scope du contenu archivé ; valeur par défaut contrôlable via `~/.claude/memory-kit.json` (`default_scope: pro` pour un poste d'entreprise, `perso` pour un poste perso).
...
2. **Filtre strict** : uniquement les fichiers avec `scope: pro` **ET** `collectif: true`.
...
- **D3.1** Pas de scope `inbox` / `brouillon` supplémentaire. Le dossier `00-inbox/` suffit à représenter le contenu non qualifié. Tout contenu hors `00-inbox/` porte obligatoirement `scope: perso` ou `scope: pro`.
- **D3.2** Le flag `collectif` reste un booléen simple en v0.5.0 (`true` / `false`). L'affinage (granularité `equipe|entreprise|public`) sera traité dans CollectiveBrain quand les cas réels seront connus — hors scope SecondBrain.
- **D3.3** `contexte.md` et `historique.md` d'un projet héritent du scope du projet. Le scope est déclaré **une seule fois** dans `contexte.md` (frontmatter du projet) et s'applique à tous les fichiers du dossier projet. Un projet est soit `perso`, soit `pro`, jamais les deux.
...
- **Skills concernés** :
  ...
  - **Aucun archive de session ne doit finir en inbox** — `mem-archive` route toujours vers `10-episodes/projets/{slug}/archives/`.
...
- **Scope applicable** : `perso` et `pro`. Le scope est déclaré au niveau du **projet ou du domaine** (dans `contexte.md`) et hérité par toutes les archives du dossier (D3.3).
- **Structure interne — deux sous-logiques symétriques** (décision 2026-04-24, option C retenue) :
  ...
  10-episodes/
    projets/                # vocation FINIE — projets avec début, déroulé, fin
      {slug}/
        contexte.md         # scope: perso|pro — snapshot mutable
        historique.md       # fil chronologique avec liens
    ...
    domaines/               # PERMANENTS — pas de fin prévue
      {slug}/               # ex: sante, famille, finances, veille-tech, hygiene-mail
        contexte.md         # snapshot mutable du domaine (état courant)
        historique.md       # fil chronologique
        archives/           # horodatées, immuables
  ```
  Un **projet** a une fin probable (même lointaine) : il vise un livrable ou une transformation bornée. Un **domaine** n'a pas de fin : c'est un fil continu d'attention (santé, relation de couple, veille tech). Les deux ont la même structure interne (`contexte.md` + `historique.md` + `archives/`), donc les skills les traitent de manière uniforme — seul le **chemin racine** change (`projets/` vs `domaines/`).

- **Règle d'apparition d'un domaine** (anti-dérive) : un domaine n'est créé que si tu as **au moins 3 archives** qui ne rentrent dans aucun projet mais se rattachent au même fil continu. Avant ce seuil, les événements isolés vivent en `00-inbox/`. Règle gravée dans `99-meta/regles-classement.md`.
...
- **Frontmatter canonique (archive projet OU domaine)** :
  ...
  projet: {slug}                    # si kind: projet
  domaine: {slug}                   # si kind: domaine
  scope: {perso|pro}
  collectif: false
...
- **Frontmatter canonique (contexte.md d'un projet ou domaine)** :
  ```yaml
  ---
  zone: episodes
  kind: {projet|domaine}
...
- **Tags attendus** : `zone/episodes`, `kind/{projet|domaine}`, `projet/{slug}` OU `domaine/{slug}`, `scope/*`, `source/*`, `modality/*`.

- **Skills concernés** :
  - `mem-archive` — écriture principale. Demande ou infère `kind` (projet ou domaine) si ambigu. Par défaut : tente match dans `projets/` puis `domaines/`, sinon crée en `00-inbox/`.
  - `mem-recall` — lit `contexte.md` + dernières entrées d'`historique.md`. Marche sur projet **ou** domaine avec la même procédure.
...
- **Frontmatter canonique** :
  ...
  zone: knowledge
  scope: {perso|pro}
  collectif: false               # true si pro et partageable via CollectiveBrain
  modality: {left|right}
...
- **Structure interne** :
  ...
  30-procedures/
    pro/
      {categorie}/      # ex: release, incident, dev, admin...
    perso/
      {categorie}/      # ex: cuisine, sante, admin-perso...
  ```
- **Frontmatter canonique** :
  ```yaml
  ---
...
### 4.5 `40-principes/` — heuristiques et lignes rouges
...
- **Frontmatter canonique** :
  ...
  modality: left
  type: principe
  tags: [zone/principes, scope/*, type/principe, {domaine}]
  force: {ligne-rouge|heuristique|preference}
  contexte_origine: ""           # texte court : incident ou lecture qui a généré ce principe
  ---
  ```
- **Tags attendus** : `zone/principes`, `scope/*`, `type/principe`, `force/*`.
- **Skills concernés** :
  - `mem-principle` (nouveau, à confirmer) — ajoute un principe.
  - `mem-recall` — les principes d'un projet (`scope: pro` + tag `projet/{slug}`) sont chargés au recall du projet pour nourrir le contexte LLM.
  - `mem-search`.

### 4.6 `50-objectifs/` — prospective et intentions
...
- **Frontmatter canonique** :
  ...
  modality: left
  type: objectif
  tags: [zone/objectifs, scope/*, type/objectif, {horizon}]
  ...
  ---
  ```
- **Tags attendus** : `zone/objectifs`, `scope/*`, `type/objectif`, `horizon/*`, `statut/*`.
- **Skills concernés** :
...
### 4.7 `60-personnes/` — carnet relationnel
...
- **Frontmatter canonique** :
  ...
  zone: personnes
  scope: {perso|pro}
  collectif: false                # invariant : si sensitive: true alors collectif: false
  ...
  modality: left
  type: personne
  tags: [zone/personnes, scope/*, type/personne, {categorie}]
  nom: "Prénom NOM"
  role: ""                        # rôle / relation
  ...
  contact:
    email: ""
    tel: ""
  derniere_interaction: YYYY-MM-DD
  ---
  ```
- **Coordonnées dans le vault** (décision Q4.4 2026-04-24) : autorisées pour les deux scopes, car le vault est personnel et local. Contrepartie : flag `sensitive: true` par défaut sur toutes les fiches `60-personnes/**`, qui **interdit mécaniquement** l'activation de `collectif: true` (enforcement dans `mem-reclass` et dans le plugin CollectiveBrain futur). L'utilisateur peut basculer `sensitive: false` s'il le souhaite, mais doit le faire explicitement.
- **Tags attendus** : `zone/personnes`, `scope/*`, `type/personne`, `categorie/*`.
- **Skills concernés** :
...
- **Rôle** : doctrine du vault, catalogues, règles de classement, index maîtres. « Comment le vault est-il organisé, selon quelles conventions ? »
- **Contenu type** : `_index.md` global, doctrine du vault (copie du présent document), glossaire des tags, règles de classement, conventions d'encodage, journaux de refonte.
- **Exemples** :
  - `99-meta/_index.md` (catalogue maître du vault)
  - `99-meta/doctrine.md` (copie archivée de brain-architecture-v0.5)
  - `99-meta/taxonomie-tags.md`
  - `99-meta/regles-classement.md`.
...
- **Skills concernés** :
  - `mem-recall` lit `_index.md` (chargement du catalogue projets).
...
Les atomes peuvent **se référencer entre eux** via les liens Obsidian `[[...]]` (ex: l'archive de session cite les principes nouveaux extraits, qui eux-mêmes pointent vers l'archive comme `contexte_origine`).
...
| 4 | Règle / contrainte / valeur (« toujours », « jamais », « privilégier ») | `40-principes/` |
| 5 | Intention future + horizon temporel (« objectif », « d'ici », échéance) | `50-objectifs/` |
| 6 | Fiche personne (nom propre + rôle/relation) | `60-personnes/` |
...
Plan d'ingestion (3 atomes détectés) :
  [1] "Le commit fix: no force-push a été motivé par..." 
      → 10-episodes/projets/secondbrain/archives/2026-04-24-... (source: archeo-git)
  [2] "Règle : ne jamais force-pusher sur main"
      → 40-principes/pro/dev/no-force-push.md (nouveau principe)
...
- Atome 1 : **événement daté** → archive dans `10-episodes/projets/secondbrain/archives/YYYY-MM-DD-...archeo-git.md` (source archeo-git, contenu factuel : hash, auteur, fichiers modifiés, diff stats).
- Atome 2 : **principe extrait** → nouvelle fiche `40-principes/pro/dev/no-force-push-sur-main.md` (si le principe n'existe pas déjà, sinon enrichir l'existant avec `contexte_origine` pointant vers l'archive).
...
Idem pour `mem-archeo-atlassian` : une page Confluence peut contenir à la fois du factuel (historique d'un incident → `10-episodes`), des principes (règles d'écriture du code produit → `40-principes`), et des connaissances (glossaire métier → `20-knowledge`). Le router segmente et distribue.

**Conséquence sur l'idempotence** : l'idempotence des skills archeo (ne jamais recréer une archive déjà ingérée) doit s'étendre aux atomes dérivés. Si un jalon a déjà généré un principe `P`, une ré-ingestion ne doit pas créer `P2` en doublon — elle doit détecter `P` existant et, soit le laisser intact (si identique), soit l'enrichir (si évolution). Implémenté via les champs d'origine : `derived_from: <archive_id>` dans les atomes dérivés, et recherche par `(source_jalon + type_atome + sujet)` lors de la ré-ingestion.
...
- **~~Q5.3~~** ✅ **décidé 2026-04-24** : les atomes dérivés sont liés **bidirectionnellement** via Obsidian `[[...]]`. L'archive source cite les atomes extraits (« principes dégagés : [[no-force-push-sur-main]] »), et chaque atome extrait pointe vers l'archive (`contexte_origine: [[2026-04-24-...]]`). Obsidian Graph rendra visible la filiation.
...
| `projet/{slug}` | slug du projet | si `kind: projet` ou rattachement transverse | Rattachement à un projet. Transverse : un principe peut porter `projet/secondbrain` pour indiquer son rattachement origine. |
| `domaine/{slug}` | slug du domaine | si `kind: domaine` ou rattachement transverse | Rattachement à un domaine permanent (santé, famille, veille-tech...). |
...
- **Hiérarchie** : **un seul niveau de `/` par tag** (`scope/pro` OK, `scope/pro/entreprise` non — si besoin de granularité, créer un namespace dédié).
...
Les namespaces permettent autant d'axes de coloration indépendants dans le plugin Extended Graph (cf. chantier annexe dans `contexte.md` SecondBrain). Suggestion de configuration, hors scope de ce document :

- **Couleur de nœud** = `zone/*` (9 couleurs pour les 9 zones)
- **Couleur de bordure / halo** = `scope/*` (2 teintes pour perso/pro)
- **Taille de nœud** = présence de `kind/projet` (projets plus visibles que domaines)
- **Arcs transverses** = `projet/{slug}` et `domaine/{slug}` (met en évidence la filiation entre atomes d'un même fil)
...
Les champs `scope`, `collectif`, `modality` sont **absents** (à fixer au reclassement).
...
zone: episodes
kind: projet                # ou: domaine
projet: {slug}              # si kind: projet
domaine: {slug}             # si kind: domaine
scope: {perso|pro}
collectif: false
modality: left
source: vecu                # ou: doc | archeo-git | archeo-atlassian | manuel
type: archive
tags: [zone/episodes, kind/projet, projet/{slug}, scope/*, modality/left, source/vecu, type/archive]
derived_atoms: []           # optionnel — atomes dérivés par le router (section 5.6)
---
```

#### `10-episodes` — `contexte.md` d'un projet ou domaine
...
#### `40-principes` — principe / heuristique / ligne rouge
...
type: principe
force: {ligne-rouge|heuristique|preference}
contexte_origine: "[[YYYY-MM-DD-...]]"  # lien vers archive fondatrice si applicable (Q4.3)
projet: {slug}              # optionnel, si rattaché à un projet origine
tags: [zone/principes, scope/*, type/principe, force/*, categorie/*]
---
```

#### `50-objectifs` — objectif / intention
...
tags: [zone/objectifs, scope/*, type/objectif, horizon/*, statut/*]
---
```

#### `60-personnes` — fiche personne
...
scope: {perso|pro}
collectif: false            # invariant : si sensitive: true alors collectif: false
sensitive: true             # défaut — interdit mécaniquement collectif: true (Q4.4)
modality: left
type: personne
nom: "Prénom NOM"
role: ""                    # rôle / relation
...
Pas de `scope`, `collectif`, `modality` (méta neutre, transverse).
...
2. **Sensitive true ⇒ collectif false.** Si `sensitive: true` (personnes par défaut), `collectif: true` interdit.
3. **Zone episodes ⇒ `kind` présent.** Jamais d'archive sans `kind: projet` ou `kind: domaine`.
4. **Kind projet ⇒ `projet: {slug}` présent et slug existant dans `10-episodes/projets/`.** Idem pour `kind: domaine`.
...
**Atome 1 — archive du commit** (`10-episodes/projets/mcp-iris-connector/archives/2026-01-15-14h30-mcp-iris-connector-archeo-tag-v0-3-1.md`) :
...
tags: [zone/episodes, kind/projet, projet/mcp-iris-connector, scope/pro, modality/left, source/archeo-git, type/archive]
---
```

**Atome 2 — principe extrait** (`40-principes/pro/dev/no-force-push-sur-main.md`) :
...
type: principe
force: ligne-rouge
contexte_origine: "[[2026-01-15-14h30-mcp-iris-connector-archeo-tag-v0-3-1]]"
projet: mcp-iris-connector
tags: [zone/principes, projet/mcp-iris-connector, scope/pro, modality/left, type/principe, force/ligne-rouge, categorie/dev]
---
```

Les deux atomes sont **liés bidirectionnellement** (décision Q5.3) via `derived_atoms` et `contexte_origine`, rendant la filiation visible dans Obsidian Graph.
...
| `mem-recall` | ♻️ refonte | Élevé | Charge `contexte.md` + dernières entrées `historique.md` d'un projet OU domaine (argument). Charge aussi : **principes actifs** rattachés (`tag: projet/{slug}`), **objectifs ouverts** (`statut: ouvert\|en-cours` + projet), **personnes-clés** récemment interagies. |
...
| `mem-archeo` | ♻️ refonte | Élevé | **Plus d'archive monolithique** : chaque jalon Git produit plusieurs atomes via le router. Idempotence élargie : `source_jalon + type_atome + sujet` évite les doublons aux ré-ingestions. |
...
| **`mem-promote-domain` (nouveau)** | 🆕 | — | Promeut un ensemble d'items cohérents de `00-inbox/` vers un nouveau `10-episodes/domaines/{slug}/`. Vérifie la règle anti-dérive (≥ 3 items au même fil). |
...
- **Extensible** à d'autres blocs communs : `_encoding.md` (la directive UTF-8 sans BOM), `_concurrence.md` (les patterns atomic write + hash check), `_frontmatter-universel.md`.
...
Au moment de la migration (fin 2026-04 prévue), le vault contient environ **50 archives** dans `archives/` (plat) et **7 projets** sous `projets/` (codemagdns, collectivebrain, gabrielle, gmt-knowledges, iris-sync, mcp-iris-connector, secondbrain). Aucun autre contenu (pas de notes de connaissance, pas de principes formalisés, pas d'objectifs explicites, pas de fiches personnes). Le refactor transforme cette structure project-centric en structure brain-centric.

### 9.2 Mapping v0.4 → v0.5

| v0.4 | v0.5 | Règle |
|---|---|---|
| `archives/YYYY-MM-DD-HHhMM-{slug}-{sujet}.md` (source `vecu\|doc\|archeo-*`) | `10-episodes/projets/{slug}/archives/YYYY-MM-DD-HHhMM-{slug}-{sujet}.md` | Le slug projet est extrait du frontmatter `projet:` ou du nom de fichier. Toutes les archives d'un projet sont déplacées dans son sous-dossier `archives/`. |
...
| `projets/{slug}/contexte.md` | `10-episodes/projets/{slug}/contexte.md` | Déplacement tel quel. Le frontmatter est enrichi avec `zone: episodes`, `kind: projet`, `slug`, `scope` (défaut `pro`, à confirmer manuellement). |
| `projets/{slug}/historique.md` | `10-episodes/projets/{slug}/historique.md` | Déplacement tel quel. Les liens relatifs `../../archives/...` sont réécrits en `archives/...` (archives désormais scopées au projet). |
| `_index.md` (actuel) | `99-meta/_index.md` | Déplacement + refonte : l'index doit refléter la nouvelle arborescence (liste par zone, plus par projet seulement). |
...
| `collectif` | `false` (toujours à la migration) |
...
3. Créer arborescence v0.5 dans le vault :
   00-inbox/ 10-episodes/projets/ 10-episodes/domaines/
   20-knowledge/ 30-procedures/ 40-principes/ 50-objectifs/
   60-personnes/ 70-cognition/ 99-meta/
4. Pour chaque projet dans {vault}/projets/ :
   a. Déplacer contexte.md, historique.md vers 10-episodes/projets/{slug}/
...
5. Pour chaque archive dans {vault}/archives/ :
   ...
   b. Déterminer destination : 00-inbox/ ou 10-episodes/projets/{slug}/archives/
...
6. Déplacer {vault}/_index.md vers {vault}/99-meta/_index.md, refonte structurelle.
...
- [ ] `archives/` et `projets/` supprimés (ou vides).
- [ ] `10-episodes/projets/` contient 7 sous-dossiers (codemagdns, collectivebrain, gabrielle, gmt-knowledges, iris-sync, mcp-iris-connector, secondbrain).
- [ ] Chaque projet a `contexte.md`, `historique.md`, `archives/` (non vide).
...
- [ ] `99-meta/_index.md` présent, structure à jour.
...
2. **Blocs réutilisables** dans `core/procedures/` :
   ...
   - `_frontmatter-universel.md` (réf aux 6 champs universels section 7.1)
...
- **CollectiveBrain v0.1** (projet séparé) : plugin Obsidian qui lit les atomes `pro` + `collectif: true` et synchronise vers le vault collectif sur GMT Knowledges.
