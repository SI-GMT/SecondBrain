...
Si l'outil MCP **`mcp__secondbrain-memory-kit__{{TOOL_NAME}}`** est disponible dans tes outils (cas Claude Code / Codex / Copilot CLI / Gemini CLI / Mistral Vibe quand le serveur `secondbrain-memory-kit` est configuré et démarré) :
...
**Comment savoir si l'outil est disponible** : il apparaît dans la liste de tes outils sous le préfixe `mcp__secondbrain-memory-kit__` (ou équivalent selon le naming de ta CLI). Si tu ne le vois pas, c'est qu'il n'est pas configuré → bascule en skills mode.

### Skills fallback (CLI sans MCP, ou serveur indisponible)

Si l'outil MCP n'est pas disponible (CLI sans MCP configuré, ou bien serveur `secondbrain-memory-kit` non installé / non démarré sur ce poste), **exécute la procédure complète ci-dessous** comme avant la v0.8.0. C'est le mode skills classique : tu lis et écris les fichiers du vault toi-même selon la spec.
