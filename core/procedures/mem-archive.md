...
- **Full mode**: validates the new archive body AND the rewritten context body. If any wikilink fails to resolve, the operation aborts with a `DanglingWikilinkError` (a `ValueError` subclass) listing the offending targets — nothing is written.
- **Incremental mode**: validates the rewritten context body. Same abort behaviour on violation.

Resolution rule mirrors `mem-health-scan`: a wikilink target resolves if a file with stem `X` exists anywhere in the vault, OR a relative path `X.md` exists. The bare target `index` is always considered resolved.

**Bypass for legitimate non-resolving citations** (per `_linking.md`):

| Use case | Convention |
|---|---|
| Cite a wikilink as a literal example (doctrine, spec, tutorial) | Inline backticks: `` `[[no-force-push-sur-main]]` `` |
| Note a future target to be created later | Inline backticks + TODO note: `` `[[future-target]]` (TODO once X is documented) `` |

Wikilinks inside fenced code blocks (```` ```...``` ````) and inline code spans are filtered out before validation, so the doctrinal bypass works as documented.

**No `--allow-dangling` opt-in.** The doctrine is unambiguous: silent persistence of a dangling wikilink is a bug. The LLM client must correct the body in one pass, either by creating the missing target first OR by demoting the link to backticks.

The MCP-native enforcement lives in `mcp-server/src/memory_kit_mcp/tools/archive.py` (function `_enforce_wikilinks`) and shares its parsing/resolution code with `mem_health_scan` via `memory_kit_mcp.vault.wikilinks` — both consumers apply exactly the same rules.
