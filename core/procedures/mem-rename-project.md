...
Pour un **renommage de fichier** (pas un write), utiliser directement `mv` / `Move-Item -Force` / `Path.rename()` — ces opérations sont déjà atomiques côté FS.

### Pattern 2 — Hash check read-before-write (pour les fichiers partagés)

Les fichiers partagés modifiables par plusieurs procédures (`_index.md`, `historique.md`, `contexte.md`, frontmatters d'archives) exigent un hash check avant toute réécriture :
