...
## Categories audited (8 vault + 2 kit-repo = 10 total in MCP mode)
...
| `mcp-tool-spec-drift` | info | _(MCP mode only — v0.9.0)_ Procedure body in `core/procedures/mem-X.md` diverges from the hash recorded in `mcp-server/src/memory_kit_mcp/sync.json` — signals the matching `tools/X.py` may need review. Skipped silently if `kit_repo` not configured or `sync.json` absent. |
| `skill-description-too-long` | warn | _(MCP mode only — v0.9.x)_ SKILL.md frontmatter `description` exceeds the 1024-char Anthropic limit, which breaks the skill's auto-trigger on Claude Code / Codex / Vibe / Copilot. Audits `kit_repo/adapters/{claude-code,codex,mistral-vibe,copilot-cli}/skills/`. Gemini excluded (TOML format, no equivalent limit). Skipped silently if `kit_repo` or `adapters/` absent. |

The first 8 categories are mirrored exactly in `scripts/mem-health-scan.py` (versioned standalone CLI) — the standalone is the canonical source of vault-audit rules. Categories 9 and 10 are MCP-only by design: they audit the **kit repo** (procedures vs Python implementations, then adapter SKILL.md templates), not the vault, so the standalone — which scans vaults from machines without the kit installed — has no equivalent.
